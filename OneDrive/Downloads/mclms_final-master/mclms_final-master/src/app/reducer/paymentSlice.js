...
export const getUserReceipts = createAsyncThunk(
  `${namespace}/getUserReceipts`,
  async (id, { rejectWithValue }) => {
    try {
      axios.defaults.headers = {
        "Content-Type": "application/json",
      };
      const response = await axios.get(`${API_URL}api/receipts/user/${id}`);
      return response.data;
    } catch (err) {
      if (!err.response) {
        throw err;
...
export const getReceiptsAdmin = createAsyncThunk(
  `${namespace}/getReceiptsAdmin`,
  async (payload, { rejectWithValue }) => {
    try {
      axios.defaults.headers = {
        "Content-Type": "application/json",
...
export const updateReceiptStatus = createAsyncThunk(
  ...
  async (payload, { rejectWithValue }) => {
    try {
      ...
      console.log(payload);
      const response = await axios.put(
        `${API_URL}api/receipts/${payload.id}`,
        payload
      );
      console.log(response.data);
      return response.data;
...
export const delinquent = createAsyncThunk(
  `${namespace}/delinquent`,
  async (payload, { rejectWithValue }) => {
    try {
      axios.defaults.headers = {
        "Content-Type": "application/json",
      };
      const response = await axios.get(`${API_URL}api/payments/delinquents`);
      console.log(response.data);
      return response.data;
    } catch (err) {
      if (!err.response) {
...
export const getDelinquent = createAsyncThunk(
  `${namespace}/getDelinquent`,
  //payload is id
  async (payload, { rejectWithValue }) => {
    try {
      axios.defaults.headers = {
      ...
      const response = await axios.get(`${API_URL}api/payments/delinquents/${payload}`);
      console.log(response.data);
      return response.data;
    } catch (err) {
      if (!err.response) {
...
export const paymentSlice = createSlice({
  ...
  extraReducers: {
    ...
    [getUserReceipts.pending]: (state) => {
      state.loading = true;
      state.error = null;
    },
    [getUserReceipts.fulfilled]: (state, { payload }) => {
      state.error = null;
      state.loading = false;
      state.userReceipts = payload;
    },
    [getUserReceipts.rejected]: (state, { payload }) => {
      state.loading = false;
      state.error = payload;
      console.log(payload);
    },
    [getReceiptsAdmin.pending]: (state) => {
      state.loading = true;
      state.error = null;
    },
    [getReceiptsAdmin.fulfilled]: (state, { payload }) => {
      state.receipts = payload;
      state.error = null;
      state.loading = false;
    },
    [getReceiptsAdmin.rejected]: (state, { payload }) => {
      state.loading = false;
      state.error = payload;
      console.log(payload);
    },
    [updateReceiptStatus.pending]: (state) => {
      state.loading = true;
      state.error = null;
    },
    [updateReceiptStatus.fulfilled]: (state, { payload }) => {
      state.receipts = payload;
      state.error = null;
      state.loading = false;
    },
    [updateReceiptStatus.rejected]: (state, { payload }) => {
      state.loading = false;
      state.error = payload;
      console.log(payload);
    },
    [delinquent.pending]: (state) => {
      state.loading = true;
      state.error = null;
    },
    [delinquent.fulfilled]: (state, { payload }) => {
      state.error = null;
      state.loading = false;
    },
    [delinquent.rejected]: (state, { payload }) => {
      state.loading = false;
      state.error = payload;
      console.log(payload);
    },

    [getDelinquent.pending]: (state) => {
      state.loading = true;
      state.error = null;
    },
    [getDelinquent.fulfilled]: (state, { payload }) => {
      state.error = null;
      state.loading = false;
    },
    [getDelinquent.rejected]: (state, { payload }) => {
      state.loading = false;
      state.error = payload;
    },
